...
QObject::connect(ns->getNotebook(notebookGuid), &AsyncResult::finished, [](QVariant result, QSharedPointer<EverCloudExceptionData> error) {
 ...
 @endcode
%Note the handling of the `EDAMSystemExceptionRateLimitReached` and `EDAMSystemExceptionAuthExpired` cases. They should be handled
for any Evernote API function call in a special way.
